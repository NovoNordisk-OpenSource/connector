...
get_backend_fct <- function(backend_type) {
  ...
  if (backend_type %in% defaults_backends) {
    ...
    return(getExportedValue(package_name, backend_type))
  }
}
...
#'
create_backend_fs <- function(yaml_content, backend) {
  ...
  custom_path <- extract_custom_path(yaml_content, path)

  params_from_user$path <- custom_path
