...
``` r
connector_fs(path, extra_class = NULL, metadata = NULL)
```
...
- metadata:
  ...
  [list](https://rdrr.io/r/base/list.html) Optional metadata to attach
...
The `metadata` parameter allows you to attach additional information. By
default, it is stored as an attribute named "metadata" on the connector
object.
