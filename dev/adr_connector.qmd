...
This documentation will explain why we will use a specific implementation. It is a support for a discussion to take the best decision a the moment with what we know today.

-   Understand the need for the connector package
-   Investigate the best way of implementing the package
-   Give an overview of all possibilities R6, S7 or something else ?

We will give examples only for ADAM data sets and two sources of data sets : File system, Databricks
...
After years with p-Drive and the package NNaccess, Novo Nordisk have decided to change the infrastructure of clinical trails. **We will use Databricks to store and interact with data sets.**

In this context, we have rethink the NNaccess package to be more general and be able to use any kind of databases or file system file. The inspiration comes to the DBI package it gives you the possibility this the same package to use different kind of databases.
...
DBI is based on S4 and give the opportunity to create a new class to a new database:
...
Our aim is to imitate the DBI package but for more diverse and varied systems, such as a filesystem or databricks.

> We want to make it possible to use different backends but with the same interface. The API for a user will be the same for all the backends.

The inspiration comes from the DBI package, but also from the [interfaces](https://medium.com/@jamal.kaksouri/understanding-the-power-of-go-interfaces-a-comprehensive-guide-835954101b7e) in golang.

## Try first draft with S3 and S7

### Testing around the best implementation

This part is only here to test some code and see how it works.
...
Dispatch methods looks like a good idea to implement the connector package.
...
#### Dispatching different methods according to a new configuration

How can you benefit from dispatching different methods depending on the backend and configuration?
...
In conclusion, technically, using S7 is a possibility but it is not the best way to implement the connector package. It is too complex and not really adapted to the need of the package. The configuration class is too linked to the backend class and it is not really flexible. We will not go for a full solution with S7.
...
## Combination of R6 and S3, a structured object with generics methods

### A first project to illustrate the idea of using R6
...
Using only R6 will require to define a new object for differents backend. It is not really flexible and not really adapted to the need of the package.

### Implementing a R6 class for file system with S3 dispatch
...
The create layer function give us the opportunity to add a specific class to the layer object. This is a way to add a specific method to a specific layer or a specific *FS* backend. You can also notice that we have a builder parameter. This is a way to define a new **fs** builder for our specific needs. Because of inheritance, this object have a "fs" class and allow us to define defaults methods for all file system backend.

#### Using S3 to dispatch methods

Define default methods for the file system backend :
...
This example is already a bit complex because our read.fs method is also a generic method that will give us the opportunity to define methods for different kind of files: csv, parquet, rds and so on.
...
#### Concrete example for ADAM dataset

So, we are now able to use our object with the methods we have defined to read and write adam datasets.
...
#### Giving only a yaml file or a list to configure the backend

```{r}
fs <- create_layer_fs

### Small example of a connector function
...
### Our configuration for the backend
config <- list(
    trial = "id",
    project = "numtrial",
    root_path = system.file("trials", package = "connector"),
...
### Reuse the same logic for a DBI connection : smooth transition between different backend
...
create_layer_dbi <- function(layer, conn, extra_class = NULL, builder = builder_dbi) {
    layer_ <- builder$new(layer, conn)
...
This our new builder for dbi connection. We have the same methods as for the file system backend. We can define a new class for the dbi backend and add specific methods for this backend.

Example of a specific method for the dbi backend :
...
This concrete example show how we can use the same interface for different backend. We have a file system backend and a DBI backend. User will use the same methods for both backend.

So scripts will be the same for a user, only the configuration will change.

### The extra class implementation to define a specific backend or configuration

What happen if a compagny want to add a specific method for a specific backend or a specific layer ?

> They can use the extra class parameter to modify a specific method for a specific backend.

For now, we have define extra class to be design such as :

**extra class_previous class**

For a DBI example :

If you want a extra class for a company, you will use a the extra class param :

**extra_class = "my_company"**

And you will have a specific class for your company for the DBI backend : **my_company_dbi**

If it was for fs, you will have : **my_company_fs**

Here a concrete example :

-   Create a extra class for a specific stdm
...
-   Because of the inherit class, read and write work
...
-   You can overwrite methods by using the class name
...
##### About a specific method for a specific layer
...
This small example show, based on a yaml file, how we can create a connection object. We will have to define the business logic for the trial and the backend.
...
```{r}
adam_new <- create_layer_fs("adam", system.file("trials", "id_numtrial", "adam", package = "connector"),
 builder = new_fs)
...
So as a developer, you just have to create a new builder with the previous builder as parent and add a new method to it. Everything related to the ***fs*** backend still works. But you can customize your own standard methods for reading, writing, and so on.
...
3.  Third option is to add a new method to the ***fs*** object generator itself.

```{r}
builder_fs$set("public","get_info", function(name) {
    message("get info for a file")
    file.info(file.path(self$root_path, name))
})
```

This method will be available for all ***fs*** backend for the package used. In a way, this is a best way to ad a new method if is for all users. But if it is only for you, the first option is the best way and less intrusive.

### What about a different configuration for a other compagny ?

Last but not least, how to add a new method for a specific compagny ?

This is probably the most complex part of the package. Define our own schema for trials depends on the compagny.

We also have to define some rules for different backend.

-   If we are using FileSystem, we have decide that at the end we need a path to the folder. No matter what is the configuration for the compagny, we need a path to the folder.
-   If we are using a database, we will work around a logic like this:
    -   If you give a path to a file, we will use it.
    -   If you give a collection, we will list schemas
    -   If you give a schema, we will list tables

We could also working around keyword for the configuration. For example, if we have a keyword **path_to_file**, we will use it for the connection.

##### Priority for backend implementation

This is the list of priority for the backend implementation:

1.  FileSystem
2.  databricks tables/ databricks volumes
3.  Postgres (DBI)
4.  MongoDB (implemented by someone outside form the project)

#### For company business logic

The idea behind is to define a generic way to interact with specific company configuration.

Because of R, we will use a "glue-like" specification for the configuration:

**{root_path}/{compound}*{indication}/{compound}*{indication}*{trial}*{instance}/{level}**/*{ifelse(ext != "sas7bdat", ext, "")}/{object}.{ext}*

For example, for a company, we will have a configuration like this :
...
connector:
  company:
    ...
    trial: "trial"
    instance: "inst"
    path_definition: {root_path}/{compound}_{indication}/{compound}_{indication}_{trial}_{instance}
  layers:
    adam:
      conn: fs
        compound: "test"
        ...
        trial: "trial"
        instance: "inst"
        path_definition: {root_path}/{compound}_{indication}/{compound}_{indication}_{trial}_{instance}/stats/data/{layer}
    stdm:
      conn: fs

```

This is a simple example of a configuration for a company. We have a compound, an indication, a trial and an instance. The path definition will give us the information to get the trial folder.

Based on this idea, we will try to implement this for 3 differents companies with different configuration.

##### Company 1

The novonordisk way.

pdrive/nn2123/nn2123-0001/current

##### Company 2

```         
system_root/{compound}_{indication}/{compound}_{indication}_{trialnr}_{instance}
