...
After years with pDrive and the package NNaccess, NovoNordisk have decided to change the infrastructure of clinical trails. **We will use Databricks to store and interact with datasets.** 
...
## Combinaison of R6 and S3, a structured object with generics methods
...
This small example show, based on a yaml file, how we can create a connection object. We will have to define the buisness logic for the trial and the backend.
...
This is the list of priority for the backend implementation :
    - FileSystem
    - databricks tables/ databricks volumes
    - Postgres (DBI)
    - MongoDB (implemented by someone outside form the project)

#### For company buisness logic
...
**{root_path}/{compound}_{indication}/{compound}_{indication}_{trial}_{instance}/{level}**/*{ifelse(ext != "sas7bdat", ext, "")}/{object}.{ext}*
