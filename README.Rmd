...
The connector package comes with the possibilities of creating connections to file system folders using `ConnectorFS()` and general databases using `ConnectorDBI()`,
...
The first we name "folder", specify the type to be `ConnectorFS()`, and the path to the folder.
The second is a database connector to an in memory SQLite database, that we specify using the `ConnectorDBI()` type,
...
Here the parquet format has been used, but when using a `ConnectorFS()` it is possible to read and write several different file types. 
