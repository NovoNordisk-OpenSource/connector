...
parent class has no methods for e.g. reading (`read_cnt()`) or writing (`write_cnt()`) data.

In most cases you want to inherit from either `connector_fs()` or `connector_dbi()` depending
...
When we not initialize a `connector_project` you can see that it still has all the methods from `connector_fs()`,
...
We can now use this `connector` to read and write data, just as we would with `connector_fs()`:
...
This means that you can create custom methods for your new connector class, and they will be used when the generic is called, 
...
to be used instead of `list_content_cnt.connector_fs()`:
...
This is of course a very simple example, that just prints a message before calling the `connector_fs` method.
...
To redo the two examples above we make a new `connector_fs` with the extra class `demo_extra`:
...
```
As you can see here we have all the methods from `connector_fs()`, but the `demo_extra` is now the first class in the class hierarchy.
...
For the file storage connectors (inheriting from `connector_fs()`) the `read_file()` method is 
