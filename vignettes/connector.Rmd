...
connections:
  - con: "adam"
    backend:
        type: "fs"
        path: "{root_path}/{project}/{trial}/{instance}/adam"
  - con: "tfl"
    backend:
...
## Store data
db$adam %>%
  cnt_write(setosa, "setosa", overwrite = TRUE)

db$adam %>%
  cnt_write(mean_for_all_iris, "mean_iris", overwrite = TRUE)

db$adam %>%
  cnt_write(cars, "cars_mpg", overwrite = TRUE)

db$adam %>%
  cnt_write(mean_for_all_mtcars, "mean_mtcars", overwrite = TRUE)
...
table <- db$adam %>%
  cnt_read("mean_mtcars")

gttable <- table %>%
  gt(groupname_col = "gear")

## Save table
db$tfl$cnt_write(gttable$`_data`, "tmeanallmtcars.csv")
db$tfl$cnt_write(gttable, "tmeanallmtcars.rds")

## Using Sharepoint
tmp_file <- tempfile(fileext = ".docx")
gtsave(gttable, tmp_file)
db$output_sh$cnt_upload(tmp_file, "tmeanallmtcars.docx")
...
## Store Tfls
db$tfl$cnt_write(fsetosa$data, "fsetosa.csv")
db$tfl$cnt_write(fsetosa, "fsetosa.rds")
...
## Using Databricks Volumes
db$tfl$cnt_upload(contents = tmp_file, file_path = "fsetosa.png")
```
...
## Store data
db$adam %>%
  cnt_write(setosa, "setosa.rds")

db$adam %>%
  cnt_write(mean_for_all_iris, "mean_iris.rds")
...
tmp_file <- tempfile(fileext = ".png")
ggsave(tmp_file, fsetosa)
db$tfl$cnt_upload(tmp_file, "fsetosa.png")

# Check if everything is written into temporary TFL directory
db$tfl$cnt_list_content()
```
## Conclusion
